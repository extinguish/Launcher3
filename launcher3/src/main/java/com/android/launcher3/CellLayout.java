...
/**
 * CellLayout就是Workspace当中的每一个Element,我们看到的每一页就是一个CellLayout
 */
public class CellLayout extends ViewGroup {
    static final String TAG = "Launcher3_CellLayout";
    ...
    // 下面的mCountX和mCountY代表当前的CellLayout的X方向和Y方向最多可以容纳的Icon的个数
    // 当然mCountX和mCountY的值可以在setGridSize()方法当中显式设定，
    // 也可以在AttributesSet当中隐式指定，
    // 如果在AttributeSet当中指定，那么就是通过DeviceProfile当中来进行调整的
    private int mCountX;
    private int mCountY;
    ...
    void setIsDragOverlapping(boolean isDragOverlapping) {
        Log.d(TAG_DIRTY, " judging whether the current drag is overlapping ... ");
    ...
    // 当我们初次启动Launcher时，向Workspace当中添加Item的过程就是在下面的这个方法当中实现的
    public boolean addViewToCellLayout(View child, int index, int childId, LayoutParams params,
                                       boolean markCells) {
        Log.d(TAG, " add view to cellLayout ... ");
        final LayoutParams lp = params;

        // Hotseat icons - remove text
        if (child instanceof BubbleTextView) {
            BubbleTextView bubbleChild = (BubbleTextView) child;
            // 当我们的CellLayout是Hotseat的实例时，我们是不显示每一个Shortcut下面的title字段的
            // 即只显示图标
            // 如果我们要显示Shortcut上面的Title的话，直接将这里重置就可以了
    ...
    public void removeAllViews() {
        Log.d(TAG, " remove all views from CellLayout ... ");
        clearOccupiedCells();
    ...
    public void removeAllViewsInLayout() {
        Log.d(TAG, " remove all views in Layout ");
    ...
    public void removeViewWithoutMarkingCells(View view) {
        Log.d(TAG, " remove view without marking cells ... ");
        mShortcutsAndWidgets.removeView(view);
    }

    // 当我们卸载程序时，才会调用下面这个方面来从CellLayout当中删除程序
    @Override
    public void removeView(View view) {
        Log.d(TAG, " remove view from the CellLayout");
    ...
    public void removeViewAt(int index) {
        Log.d(TAG, " remove view at : " + index);
    ...
    public void removeViewInLayout(View view) {
        Log.d(TAG, " remove view in layout ");
        markCellsAsUnoccupiedForView(view);
    ...
    public void removeViews(int start, int count) {
        Log.d(TAG, " remove views in bulk, and we need to remove : " + count + " totally ... ");
    ...
    public void removeViewsInLayout(int start, int count) {
        Log.d(TAG, " remove views in bulk, and we start with " + start + ", and end with : " + start + count);
        for (int i = start; i < start + count; i++) {
            markCellsAsUnoccupiedForView(mShortcutsAndWidgets.getChildAt(i));
        }
    ...
    /**
     * 每当我们在CellLayout当中移动一个ShortcutIcon时，当我们确定了
     * 这个Shortcut的最终位置，然后松手时，只要这个图标最后不是放到一个Folder当中，
     * 那么这个方法就会执行，这个方法用于将当前所有的图标的位置同步更新到DB当中，
     * 这样当我们重启系统时，这个Icon的位置还可以保存下来
     */
    private void commitTempPlacement() {
        Log.d(TAG, " the position of the shortCut has changed, and the current count value are : CountX : " + mCountX + ", CountY : " + mCountY);
        for (int i = 0; i < mCountX; i++) {
            for (int j = 0; j < mCountY; j++) {
    ...
    private static final String TAG_DIRTY = "Launcher_CellDirty";
    boolean isItemPlacementDirty() {
        Log.d(TAG, " judging whether this cell is dirty ... ");
    ...
    private class CellAndSpan {
        int x, y; // 图标的位置
        int spanX, spanY; // 图标在X方向和Y方向的拉伸程度
     ...
     */
    void onDragExit() {
        Log.d(TAG, " on current drag exit ... ");
    ...
    private static final String TAG_CELL = "CellDropChild";
    /**
    ...
    void onDropChild(View child) {
        if (child != null) {
            Log.d(TAG_CELL, " on Drop child, and this child is request new layout purpose ... ");
            LayoutParams lp = (LayoutParams) child.getLayoutParams();
            Log.d(TAG_CELL, " and the new parameter for this child are --> width : " + lp.width + "; height : " + lp.height);
            lp.dropped = true;
            child.requestLayout();
        }
    }
