...
/**
 * LauncherClings就是当我们首次安装Launcher3好了之后，我们启动了Launcher3时，会看到
 * 一个View是介绍Launcher3的相关介绍内容。
 * 著名的大牛根据Launcher当中的LauncherClings的实现技巧还开发了一个单独的Library，专门
 * 用于在应用首次安装时高亮一些这个程序的显著功能部分。以下是来自StackOverflow上面对这个基于
 * LauncherClings开发Showcase View的介绍：
 * It's designed to highlight and showcase specific parts of apps to the user with
 * a distinctive and attractive overlay. The library is great for pointing out
 * points of interest of users, gestures, or obscure but useful items.
 * The library is based on the 'Cling' View found in the Launcher on ICS and Jelly
 * Bean, but extended to be easier to use.
 * 但是不得不说Launcher3把这个强大的功能用的有点简单，我们本可以实现的更复杂，尤其是我们的
 * Launcher3提供了更为复杂的功能之后
 *
 */
