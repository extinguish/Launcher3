...
public class DragController {
    ...
    private static final String TAG = "Launcher_DragController";
    ...
    public DragView startDrag(Bitmap b, int dragLayerX, int dragLayerY,
        ...
        Log.d(TAG, " start dragging with the bitmap we created ... ");
        ...
        for (DragListener listener : mListeners) {
            // 我们在Drag时，不应该不加选择的随便都要drop进去
            // 例如当我们Drag的目标是InfoDropTarget时，我们就不应该这样了
    ...
    private void endDrag() {
        ...
        if (mDragging) {
            ...
            if (!isDeferred) {
                for (DragListener listener : mListeners) {
                    Log.d(TAG, " the listener we need to notify are : " + listener);
                    listener.onDragEnd();
                }
            }
        }
    ...
    private void drop(float x, float y) {
        Log.d(TAG, " drop the current item info at ---> X : " + x + ", Y : " + y);
        final int[] coordinates = mCoordinatesTemp;
        ...
        if (dropTarget != null) {
            ...
            if (dropTarget.acceptDrop(mDragObject)) {
                Log.d(TAG, " the current item could be accept by the DeleteZone ... ");
