...
public class AppsCustomizeTabHost extends FrameLayout implements LauncherTransitionable, Insettable  {
    ...
    public void onLauncherTransitionPrepare(Launcher l, boolean animated, boolean toWorkspace) {
        Log.d(TAG, " onLauncherTransitonPrepared invoked ... ");
    ...
    public void onLauncherTransitionStart(Launcher l, boolean animated, boolean toWorkspace) {
        Log.d(TAG, " onLauncherTransitionStarted from here ... ");
        mPagedView.onLauncherTransitionStart(l, animated, toWorkspace);
    }

    @Override
    public void onLauncherTransitionStep(Launcher l, float t) {
        Log.d(TAG, " onLauncherTransitionStep ... ");
        mPagedView.onLauncherTransitionStep(l, t);
    }

    @Override
    public void onLauncherTransitionEnd(Launcher l, boolean animated, boolean toWorkspace) {
        Log.d(TAG, " onLauncherTransitionEnd invoked from here ... ");
        ...
        if (!toWorkspace) {
            ...
            // 如果我们只是加载了当前的PagedView而没有加载同当前的PagedView相邻的PagedView,那么就会
            // 出现明显的卡顿(当然具体都现象并不是卡顿，而是显示一个空白的PagedView，这个现象在Nexus5上
            // 很明显，尤其是当我们所安装的程序比较多时)
            mPagedView.loadAssociatedPages(mPagedView.getCurrentPage());
            Log.d(TAG, " load the associated pages after the onLauncherTransitionEnd ... ");
