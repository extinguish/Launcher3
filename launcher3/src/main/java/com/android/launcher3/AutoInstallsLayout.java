...
public class AutoInstallsLayout {
    ...
    // 我们需要利用Resource class来获取到我们需要的Layout文件
    ...
    public int loadLayout(SQLiteDatabase db, ArrayList<Long> screenIds) {
        Log.d(TAG, " the current layoutId value are : " + mLayoutId);
        mDb = db;
        try {
    ...
    /**
     * 以下的这个方法会真的去解析Layout文件
     * 即通过LayoutId的值来得到Layout的XML文件，然后再利用XmlPullParser工具来解析这个XML
     * 文件，当然最终目标就是获取这个XML文件当中包含的我们所需要的Elements
     *
    ...
    protected int parseLayout(int layoutId, ArrayList<Long> screenIds)
        ...
        Log.d(TAG, " the layout file content we get are : " + parser);
    ...
    // TODO: 注意在我们最新的实现当中，文件夹被设计为不可以删除的
    // 用于解析出文件
    protected class FolderParser implements TagParser {
        ...
        public long parseAndAdd(XmlResourceParser parser)
            ...
            // 如果文件夹当中的Shortcut数目少于一个，就直接将文件夹删除掉，然后
            // 就是只剩下一个Shortcut就足够了
