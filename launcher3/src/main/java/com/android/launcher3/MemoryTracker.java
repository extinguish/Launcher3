...
public class MemoryTracker extends Service {
    ...
    // MemoryTracker并没有通过AIDL来定义相关的实现，而是直接通过继承Binder来进行实现
    // 不过这里作者创建MemoryTrackerInterface的作用更多是:
    // derive directly from Binder to implement your own custom RPC
    // protocol or simply instantiate a raw Binder object directly to use as a
    // token that can be shared across processes.
    // 但是我们看到作者并没有添加更多的相关信息，包括Stub, Proxy等信息，可见作者创建MemoryTrackerInterface
    // 并没有打算进行大量的RPC操作，而仅仅是让MemoryTrackerInterface具备了IPC的token就OK了，
    // 毕竟使用MemoryTracker的类大部分都来自不止一个Process，但是他们又不需要直接调用MemoryTracker
    // 当中定义的方法，而仅仅是MemoryTracker自己来调用自己的方法来进行跟踪，所以这里需要的仅仅就是一个
    // token，即RCP的token
    // 好了，扯淡了这么多，这就是作者仅仅继承了Binder而没有做更多的工作的原因
