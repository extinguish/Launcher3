...
// TODO: 我们可以对比以下PagedView和ViewPager之间的实现细节区别

/**
 * 首先PagedView当中直接可以容让的Child View就是Page这种组件了，
 * 而在Launcher3当中Page的直接子类都有AppsCustomizeCellLayout，
 * PagedViewCellLayout和PagedViewGridLayout
 * 其中AppsCustomizeCellLayout就是当我们在launcher 的Hotseat
 * 点击AllAppsButton时切换到的AllAppsView界面，而这个界面也是基于
 * PagedView实现的
 * <p/>
 * An abstraction of the original Workspace which supports browsing through a
 * sequential list of "pages"
 * <p/>
 * The PagedView also listens for the events of which invoked by the hierarchy
 * within this view(The PagedView instance) changed. The hierarchy changes whenever
 * a child is added to or removed from this view.
 * <p/>
 * 下面先普及一下关于ViewGroup.OnHierarchyChangeListener的接口的作用：
 * The ViewGroup.OnHierarchyChangeListener interface definition for a callback to be invoked
 * when the hierarchy within this view changed. The hierarchy changes whenever a child is added
 * to or removed from this view.
 */
public abstract class PagedView extends ViewGroup implements ViewGroup.OnHierarchyChangeListener {
    private static final String TAG = "Launcher3_PagedView";

    private static final boolean DEBUG = true;
    protected static final int INVALID_PAGE = -1;

    // the min drag distance for a fling to register, to prevent random page shifts
    // 例如，让我们对屏幕触发的是ACTION_DOWN时，我们是不需要使屏幕进行滑动的，所以我们设置
    // 这个值，就是用于这个阈值判断的，防止屏幕不停地滑动
    ...
    // 下面的DISABLE_TOUCH_INTERACTION默认是一直被置为false
    // 按照注释的解释就是指的是当我们点击一些系统内部的组件时，
    // 例如AOSP当中的Launcher当中提供了一个SearchFrame，对于这个Frame
    // 我们除了点击之外，不能拖动也不能onLongClick或者onLongPress
    // 事件，就是通过将这个值改成true就可以了
    ...
    // 以下是PagedView定义的全部的状态
    // 在Launcher2当中只有4中状态，在Launcher3当中添加了一种状态
    // 即TOUCH_STATE_REORDERING状态是新添加的状态
    ...
    // TOUCH_STATE_REST是最初的原始状态
    /**
     * Unless the TOUCH_STATE_REST state, otherwise will be the
     * message interception. Intercepted after message will be in
     * onInterceptTouchEvent onTouchEvent treatment. Mainly about
     * sliding status message. Other click Press news in would be
     * passed on to a layer of View controls. Then a similar treatment.
     */
    ...
    // 这里得到的只是一个简答的view ID,我们可以使用这个ID完成的功能目前是有限的
    // Page Indicator
    ...
    public PagedView(Context context, AttributeSet attrs, int defStyle) {
        ...
        // 在PagedView当中不允许震动反馈
        // setHapticFeedbackEnabled()是直接由View提供的一个内部回调
     ...
     */
    protected void init() {
        Log.d(TAG, " start to init all of the basic variables in the PagedView instance ");
        ...
        // here, we using the ViewConfiguration instance to retrieve all of the
        // basic information of the current PagedView instance
    ...
    protected void onAttachedToWindow() {
        ...
        // 在我们正式开始绘制PagedView,即我们每次调用onDraw()方法进行具体的绘制
        // 操作之前时，我们进行一些以下过程当中的初始化操作
        // 首先就是将PagedIndicator hook进来
        // Hook up the page indicator
        ViewGroup parent = (ViewGroup) getParent();
        ViewGroup grandParent = (ViewGroup) parent.getParent();

        //  init the page indicator group in the PagedView container
    ...
    // 这里的onChildViewAdded()方法和下面的onChildViewRemoved()方法都是定义在ViewGroup.OnHierarchyChangeListener
    // 接口当中的
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        /**
         * 下面说一下MotionEvent#getAction()方法的具体含义：
         ...
         * motion.(即开始调用PagedView的onTouchEvent方法)
         */
        ...
        /**
         * 注意，在我们通常的判断MotionEvent时，我们一般只是在判断switch(ev.getAction)就可以了，但是
         * 这里采用的却是switch(ev.getAction & MotionEvent.ACTION_MASK), 即这里采用的是对ev.getAction
         * 进行 与 运算。
         * 关于以上运算过程的具体分析：
         * The ACTION_MASK is used to separate the actual action and the pointer identifier (e.g.
         * first finger touched, second finger touched, etc.) The first 8 bits of the value returned
         * in getAction() is the actual action part, and so when you bitwise-AND it with the action
         * mask (= 11111111 = 255 = 0xff), you are left with only the action and none of the pointer
         * information.
         * Keep in mind here that & is used as an arithmetic operator (bitwise) and not a logical
         * operator (single & is a perfectly valid logical operator in Java, as is &&).`
         *
         * 总的来说就是action & MotionEvent.ACTION_MASK的作用相当于ev.getActionMasked()返回的值一样，
         * 区别就是 action & MotionEvent.ACTION_MASK得到的值就是单纯的ACTION_DOWN, ACTION_MOVE, ACTION_UP
         * 等很纯粹的事件，但是如果我们直接调用event.getAction()，那么得到的信息除了ACTION_DOWN, ACTION_MOVE等
         * 信息之外，还包含了在多点触控的情况下的关于Finger的信息，即我们可以通过event.getPointerId()还得到很多手指的
         * touch的信息，即我们通过event.getAction()得到的ACTION当中还包含了多点触控的相关信息。但是考虑到我们在
         * onInterceptTouchEvent()当中是不需要考虑关于多点触控的相关信息的，所以我们直接采用了action & MotionEvent.ACTION_MASK.
         * 其实我们这里也可以直接采用ev.getActionMasked()，效果是一样的
         * 当然我们也可以直接利用getAction() & ACTION_POINTER_INDEX_MASK来得到仅跟当前的多点触控中的手指touch
         * 的相关信息，即通过getAction() & ACTION_POINTER_INDEX_MASK是无法得到ACTION_MOVE, ACTION_DOWN等相关信息的
         *
         */
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                 ...
                 */
                // 因为我们在ACTION_DOWN当中设置了mActivePointerId = ev.getPointerId()的值，
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        // 我们在PagedView当中目前仅仅是处理单点触控的情况
        // TODO: 如果我们要加入多点触控的实现的话，将switch(action & MotionEvent.ACTION_MASK)改成
        // TODO: switch(action)就可以了，然后在具体的处理过程当中判断getActionIndex()的值来进行具体的处理和
        // TODO: 以及触发相关的事件就可以了
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_DOWN:
                /*
            ...
            case MotionEvent.ACTION_MOVE:
                if (mTouchState == TOUCH_STATE_SCROLLING) {
                    ...
                    // By using the indexOfChild() method to implement this
                    // of which will returns the position in the group of
                    // the specified child view.
                    final int dragViewIndex = indexOfChild(mDragView);

                    // 如果我们当前的View是位于DeleteZone当中时，我们应该直接改变
                    // 这个View的状态
    ...
    private void resetTouchState() {
        // 我们在onInterceptTouchEvent()和onTouchEvent()方法当中
        // 侦测到ACTION_CANCEL和ACTION_UP事件之后就会触发这个方法
        // 即，如果当前的事件不被处理的话，那么这个方法就被调用了
        // 毕竟处理之前的状态还调用了设置状态的一些过程
        releaseVelocityTracker();
        endReordering();
        mCancelTap = false;
        mTouchState = TOUCH_STATE_REST;
    ...
    protected void onScrollInteractionBegin() {
        Log.d(TAG, " onScrollInteraction Begin ... ");
    }

    protected void onScrollInteractionEnd() {
        Log.d(TAG, " onScrollInteraction End ... ");
    }
    ...
    /**
     * onGenericMotionEvent()这个方法本身是直接由View类提供的，
     * 下面是这个方法的描述:
     *
     * Implement this method to handle generic motion events.
     * <p>
     * Generic motion events describe joystick movements, mouse hovers, track pad
     * touches, scroll wheel movements and other input events(即对应来自于第三方输入设备的Motion事件，
     * 例如触摸板，鼠标，滚轮等，当然具体支持多少设备还要看设备本身的驱动的支持的情况).  The
     ...
     */
    // 下面的这个方法我们基本不会用到，因为现在的Android手机基本上不具备以下的特性，如果具备了
    // OK，那就换台手机
    @Override
    ...
    private void acquireVelocityTrackerAndAddMovement(MotionEvent ev) {
        if (mVelocityTracker == null) {
            // VelocityTracker是Android原生的内部提供的实现，
            // Velocityde的实现过程是根据系统内部用户的手势动作
            // 情况来进行捕捉和计算的
    ...
    protected void setPageHoveringOverDeleteDropTarget(int viewIndex, boolean isHovering) {
        // TODO: refer to the AOSP native implementation
        // TODO: if the current child is under the DeleteZone, we should
