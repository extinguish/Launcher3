 ...
 *
 * 注意区别FolderInfo和Folder，在 Launcher3 当中这是完全不同的两种概念,
 * 其中FolderInfo的作用是represents a folder containing shortcuts or apps
 * 目前看来，就是FolderInfo和Folder的主要区别就在于他们所可以存放的元素是不一样的，
 * 其中Folder当中存放的主要就是用户和系统产生的ICON，注意就是ICON，而不是通常意义上
 * 我们所指的ShortCut,而FolderInfo当中所存放的就是直接的同App绑定的Shortcut
 *
 */
