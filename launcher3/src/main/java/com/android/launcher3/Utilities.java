...
public final class Utilities {
    private static final String TAG = "Launcher3_Utilities";
    ...
    /**
     * Indicates if the device is running LMP(LOLLIPOP 5.0 version) or higher.
     */
    ...
    static Bitmap createIconBitmap(String packageName, String resourceName, IconCache cache,
        ...
        try {
            ...
            // Icon not found.
            Log.d(TAG, " the icon for the package : " + packageName + " are not found, and the reason goes to " +
                    " : " + e.toString());
        }
        return null;
    }
    ...
    static Bitmap createIconBitmap(Bitmap icon, Context context) {
        synchronized (sCanvas) { // we share the statics :-(
            if (sIconWidth == -1) {
                // 每当我们创建新的Bitmap时，我们都是直接将这个Bitmap的大小固定为
                // 48 X 48
                // 当然了这个值会随着设备的不同而自动进行调节，这个值本身是定义在R.dimen当中的
                initStatics(context);
            }
        }
    ...
    /**
     * Inverse of {@link #getDescendantCoordRelativeToParent(View, int[])}.
     * getDescendantCoordRelativeToParent()这个方法本身的作用是用于将子View的坐标
     * 转换成父View的坐标
     *
     * @param descendant 子View，相对于父View来说
     * @param root
     * @param coord
     * @return
     */
    ...
    // 我们在Launcher当中启动一个指定的Activity都是最终都是通过这个方法来完成最终的启动
    // 过程的
    // 所以我们在以后添加App Lock功能时，即应用锁功能时，我们就可以通过在这里添加相应的
    // 劫持代码来具体实现
    ...
    // 判断一个程序是否是系统程序
    // 主要是通过来判断这个程序是否有FLAG_DOWNLOAD来判断
    // FLAG_DOWNLOAD代表这个程序就是普通的第三方程序，我们是可以直接将这个程序卸载掉的
    static boolean isSystemApp(Context context, Intent intent) {
        Log.d(TAG, " judge whether the current app are the system built-in internal app ");
