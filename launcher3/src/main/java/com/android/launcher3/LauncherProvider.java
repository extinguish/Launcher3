...
public class LauncherProvider extends ContentProvider {
    ...
    private static final String TAG = "Launcher_LauncherProvider";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper implements LayoutParserCallback {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            Log.d(TAG, " the old version value are : " + version);
            if (version < 3) {
                Log.d(TAG, " VERSION ---> 3");
            ...
            if (version < 9) {
                ...
                Log.d(TAG, " load Favorites Database for the Hotseat ... ");
            ...
            if (version < 18) {
                Log.d(TAG, " VERSION ---> 18");
                // No-op
                version = 18;
            }

            if (version < 19) {
                Log.d(TAG, " VERSION ---> 19");
        ...
        // 每当我们安装好Launcher3时，即首次安装Launcher3时，LauncherProvider就会读取
        // res/xml/default_workspace.xml文件，而且在这个目录下面default_workspace.xml文件
        // 总共有三个，分别是default_workspace_4X4.xml, default_workspace_5X5, default_workspace_5X6
        // 当然具体的采用哪个文件是根据当前设备的的DeviceProfile文件来确定的
        // TODO: 所以，如果我们要改变Launcher3初次安装好的初始加载情况，我们只需要修改上面的文件的内容就OK了
        // TODO: 由于default_workspace.xml文件当中用到了Gallery和Camera程序，但是我们没有安装这些程序
        // TODO: 所以这也导致了AutoInstallsLayout#parseLayout()方法执行过程会发生Exception，
        // TODO: 我们需要修改以上的文件就OK了
        // 所以说以下的这个方法只有当Launcher3首次安装或者被卸载重新安装之后才会调用
        // 所以说这个方法是用于获取程序最初始的状态，而不是每次的Workspace当中的AppWidget, Shortcut和Folder
        // 的排列信息的
        // 在Launcher3当中loadFavorites()方法开始只是负责关于Workspace当中的screen
        // 的加载，而不像Launcher1当中会负责所有的事情，包括ShortcutIcon的加载顺序和制定
        private int loadFavorites(SQLiteDatabase db, AutoInstallsLayout loader) {
            Log.d(TAG, " ---- load favorites from the DB ----");
            ArrayList<Long> screenIds = new ArrayList<Long>();
            // 在这里我们通过AutoInstallsLayout来直接parse DB, 然后开始得到这个DB
            // 当中所有对应于当前Layout的所有的screenId列表的值
            // TODO: Use multiple loaders with fall-back and transaction.
            int count = loader.loadLayout(db, screenIds);

            // Add the screens specified by the items above
            Collections.sort(screenIds);
            // rank的值和ScreenId的值之间是相互对应的
            // screenId将每一个screen唯一标示了，而rank则标示了这个screen的显示顺序和位置
            int rank = 0;
            ContentValues values = new ContentValues();
            for (Long id : screenIds) {
                Log.d(TAG, " the current screenID are : " + id + ", and current rank are : " + rank);
                values.clear();
        ...
        // TODO: 正确的做法应该是我们直接在这里获取到当前系统当中所有安装的App，然后
        // TODO: 为每一个App初始化好他们需要的各种信息
        // TODO: 现在就是需要确定以下的部分是否是Launcher当中所有的Shortcut信息的来源
        private void migrateLauncher2Shortcuts(SQLiteDatabase db, Uri uri) {
            Log.d(TAG, " migrate Launcher to Shortcuts, and this is the place of which to reading out all of the basic ShortcutInfo in the Workspace ... ");
            final ContentResolver resolver = mContext.getContentResolver();
            Cursor c = null;
            int count = 0;
            int curScreen = 0;
